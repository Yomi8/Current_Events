    ...
    while True:
      ...
      # Answer is not correct
        ...
        # Not the same as anything provided (Error)
          ...
          print(
            "Your response was not any of the choices listed, please try again by choosing one of the choices listed."
          )
...
while True:
  ...
  spaces_logo = (terminal_width - max(len(line)
                                      for line in logo.split('\n'))) // 2
  spaces_options = (terminal_width -
                    max(len(line) for line in options.split('\n'))) // 2
